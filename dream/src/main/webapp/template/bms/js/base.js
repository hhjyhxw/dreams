...
$().ready( function() {
	...
	$.dialog = function (settings) {
		...
		if(settings.onOpen != null&&$.isFunction(settings.onOpen)){
		...
		function dialogClose() {
			...
			$dialog.remove();
			if(settings.onClose != null&&$.isFunction(settings.onClose)){
...
h:       高度         次要
*/
function omnipotent(linkurl,title,ULDoc,idDoc,w,h) {
	if(!w) w=screen.width-50;//screen.width-300;
	if(!h) h=screen.height-100;//screen.height-495;
	title = !title?'对话框':title;
	
	if(ULDoc){
		var ids =new Array();
		var url = "";
		if(idDoc){
			$("input[name='"+idDoc+"']",$("#"+ULDoc)).each(function(i,j){
				ids[i]=$(this).val();
			});
			url = "&idDoc="+idDoc;
		}else{
			$("input[type='hidden']",$("#"+ULDoc)).each(function(i,j){
				ids[i]=$(this).val();
			});
		}
		url = "idlist="+ids.toString()+ url;
		linkurl = linkurl+(linkurl.indexOf("?")<1?"?"+url:"&"+url);
